...
def k_cross_val(pena, selection = False, features = None, normalized = 'standard') :
    """
    5 fold cross validation.
    """
    cphs = []
    scores = []
    x1, x2, y1, y2 = get_data_set(x, y, 1/5, selection, features, normalized = 'standard')
    x2, x3, y2, y3 = get_data_set(x2, y2, 1/4, selection, features, normalized = 'standard')
    x3, x4, y3, y4 = get_data_set(x3, y3, 1/3, selection, features, normalized = 'standard')
    x4, x5, y4, y5 = get_data_set(x4, y4, 1/2, selection, features, normalized = 'standard')
